                ...
                <div class="col-12">
                  <div class="table-responsive">
                    <table id="order-listing" class="table">
                      ...
                      <c:forEach items="${loanRequest}" var="i" varStatus="j">
							<tr>
								<td>${j.count}</td>
								<td>${i.loginVO.email}</td>
								<td>${i.loanAmount}</td>
								<td>${i.loanRateVO.loanRate}</td>
								<td>${i.loanDescription}</td>
							
								<%-- <td>
									<button class="btn btn-outline-primary">
                               			<a href="verifyDocuments.html?id=${i.id}">Verify Documents</a>
                               		</button>
								</td> --%>
								
								<td>
									<c:if test="${i.status eq 'UNDER REVIEWED' }">
										<p style="color: blue">UNDER REVIEWED</p>	
									</c:if>
									<c:if test="${i.status eq 'APPROVED' }">
										<p style="color: green">APPROVED</p>	
									</c:if>
									<c:if test="${i.status eq 'REJECT' }">
